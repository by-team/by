defmodule BY.InvoicesTest do
  ...
  describe "lnpay_invoices" do
    ...
    @valid_attrs %{created_at_in_lnpay: ~N[2010-04-17 14:00:00], custom_records: %{}, descripton_bash: "some descripton_bash", dest_pubkey: "some dest_pubkey", expires_at: 42, expiry: 42, fee_msat: 42, is_keysend: true, ln_node_id: "some ln_node_id", memo: "some memo", num_satoshis: 42, payment_preimage: "some payment_preimage", payment_request: "some payment_request", r_hash_decoded: "some r_hash_decoded", settled_at: ~N[2010-04-17 14:00:00], ticket_id: "some ticket_id", wallet_id: "some wallet_id"}
    @update_attrs %{created_at_in_lnpay: ~N[2011-05-18 15:01:01], custom_records: %{}, descripton_bash: "some updated descripton_bash", dest_pubkey: "some updated dest_pubkey", expires_at: 43, expiry: 43, fee_msat: 43, is_keysend: false, ln_node_id: "some updated ln_node_id", memo: "some updated memo", num_satoshis: 43, payment_preimage: "some updated payment_preimage", payment_request: "some updated payment_request", r_hash_decoded: "some updated r_hash_decoded", settled_at: ~N[2011-05-18 15:01:01], ticket_id: "some updated ticket_id", wallet_id: "some updated wallet_id"}
    @invalid_attrs %{created_at_in_lnpay: nil, custom_records: nil, descripton_bash: nil, dest_pubkey: nil, expires_at: nil, expiry: nil, fee_msat: nil, is_keysend: nil, ln_node_id: nil, memo: nil, num_satoshis: nil, payment_preimage: nil, payment_request: nil, r_hash_decoded: nil, settled_at: nil, ticket_id: nil, wallet_id: nil}
    ...
    test "list_lnpay_invoices/0 returns all lnpay_invoices" do
      ln_pay_invoice = ln_pay_invoice_fixture()
      assert Invoices.list_lnpay_invoices() == [ln_pay_invoice]
    end

    test "get_ln_pay_invoice!/1 returns the ln_pay_invoice with given id" do
      ln_pay_invoice = ln_pay_invoice_fixture()
      assert Invoices.get_ln_pay_invoice!(ln_pay_invoice.id) == ln_pay_invoice
    end

    test "create_ln_pay_invoice/1 with valid data creates a ln_pay_invoice" do
      assert {:ok, %LNPayInvoice{} = ln_pay_invoice} = Invoices.create_ln_pay_invoice(@valid_attrs)
    ...
    test "update_ln_pay_invoice/2 with valid data updates the ln_pay_invoice" do
      ln_pay_invoice = ln_pay_invoice_fixture()
      assert {:ok, %LNPayInvoice{} = ln_pay_invoice} = Invoices.update_ln_pay_invoice(ln_pay_invoice, @update_attrs)
    ...
    test "update_ln_pay_invoice/2 with invalid data returns error changeset" do
      ln_pay_invoice = ln_pay_invoice_fixture()
      assert {:error, %Ecto.Changeset{}} = Invoices.update_ln_pay_invoice(ln_pay_invoice, @invalid_attrs)
      assert ln_pay_invoice == Invoices.get_ln_pay_invoice!(ln_pay_invoice.id)
    end

    test "delete_ln_pay_invoice/1 deletes the ln_pay_invoice" do
      ln_pay_invoice = ln_pay_invoice_fixture()
      assert {:ok, %LNPayInvoice{}} = Invoices.delete_ln_pay_invoice(ln_pay_invoice)
      assert_raise Ecto.NoResultsError, fn -> Invoices.get_ln_pay_invoice!(ln_pay_invoice.id) end
    end
